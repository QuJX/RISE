...
class QM9_SEGNN(InMemoryDataset):
    ...
    def __init__(self, root, target, radius, partition, lmax_attr, feature_type="one_hot"):
                 # edge_noise_std = 0, noise_class = 'Node_changed'):
    ...
    def processed_file_names(self) -> str:
        return ["_".join([self.partition, self.target + "_r="+str(np.round(self.radius, 2)),
                          self.feature_type, "l="+str(self.lmax_attr)  +'.pt'])]
                        #   "noise="+str(self.edge_noise_std), "noise_class=" + 
                        #   str(self.noise_class) +'.pt'])]
    ...
    def process(self):
        ...
        print("Processing", self.partition, "with radius=" + str(np.round(self.radius, 2)) +
              ",", "l_attr=" + str(self.lmax_attr), 
              "and", self.feature_type, "features.")
        ...
        for i, mol in enumerate(tqdm(suppl)):
            ...
            # if self.edge_noise_std > 0 and self.noise_class == 'Edge_changed':
            #     # Convert the DataFrame to a tensor
            #     noise = torch.tensor(np.random.normal(0, self.edge_noise_std, size=pos.shape), dtype=torch.float)
            #     pos = torch.tensor(pos + noise, dtype=torch.float)

            edge_index = radius_graph(pos, r=self.radius, loop=False)

            # if self.edge_noise_std > 0 and self.noise_class == 'Node_changed':
            #     # Convert the DataFrame to a tensor
            #     noise = np.random.normal(0, self.edge_noise_std, size=pos.shape)
            #     pos = torch.tensor(pos + noise, dtype=torch.float)
