...
def save_dict_to_csv(data_dict, root, filename):
    # Ensure all lists are the same length
    max_len = max(len(v) for v in data_dict.values())
    for key in data_dict:
        if len(data_dict[key]) < max_len:
            data_dict[key].extend([None] * (max_len - len(data_dict[key])))
    
    # Compute column means
    column_means = {key: np.nanmean(data_dict[key]) for key in data_dict}
    ...
    with open(root + '/' + filename, mode='w', newline='') as file:
        ...
        # Write mean row
        writer.writerow([column_means[key] for key in headers])